#!/usr/bin/env python3
"""
Download CSP reference datasets and publish Hugging Face DatasetDict repos.

This script follows the original CSP data preparation flow:
...
def main() -> None:
    p = argparse.ArgumentParser(
        description="Download CSP reference datasets, convert to HF DatasetDict, and optionally push to Hub."
    )
    p.add_argument("--data-dir", type=Path, default=Path(__file__).resolve().parent / "data")
    p.add_argument("--namespace", type=str, default="zzq1zh")
    p.add_argument("--repo-prefix", type=str, default="csp-ref")
    p.add_argument("--only", choices=("all", "mit-states", "ut-zappos", "cgqa"), default="all")
    p.add_argument("--download", action="store_true", help="Run CSP-style downloads.")
    p.add_argument("--prepare", action="store_true", help="Extract/reorganize downloaded files.")
    p.add_argument("--push", action="store_true", help="Push converted datasets to the HF Hub.")
    p.add_argument("--private", action="store_true", default=True, help="Create private Hub repos.")
    p.add_argument("--token", type=str, default=(os.environ.get("HF_TOKEN") or "").strip())
    args = p.parse_args()
    ...
    targets = list(DATASET_ROOTS.keys()) if args.only == "all" else [args.only]
    for name in targets:
        ...
        if args.push:
            ...
            url = push_dataset(ds, repo_id, private=bool(args.private), token=args.token or None)
