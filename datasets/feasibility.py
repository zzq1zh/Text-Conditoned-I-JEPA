...
from datasets.read_datasets import DATASET_TO_VISION_KEY
...
if __name__ == "__main__":
    ...
    if config.dataset not in DATASET_TO_VISION_KEY:
        known = ", ".join(sorted(DATASET_TO_VISION_KEY))
        raise KeyError(f"Unknown dataset {config.dataset!r}. Choose one of: {known}")
    dataset =  CompositionDataset(config.dataset,
                                    phase='test',
