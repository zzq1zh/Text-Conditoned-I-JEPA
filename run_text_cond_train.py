...
def _parse_args() -> tuple[argparse.Namespace, list[str]]:
    p = argparse.ArgumentParser(
        description="Unified training launcher for {ijepa, vjepa, dinov3} on cspref_mit_states with clip_similarity fusion."
    )
    p.add_argument(
        "--vision-backbone",
        required=True,
...
def main() -> None:
    ...
    # Training hyperparameters from hyperparameters.json unless overridden by CLI (e.g. --epochs).
    ...
    for seed in seeds:
        ...
        print(f"Running train command (seed={seed}):", flush=True)
        print(" ".join(train_cmd), flush=True)
        print(f"Checkpoint will be saved to: {ckpt}", flush=True)
        print(f"Running eval command (val):", flush=True)
        print(" ".join(eval_val_cmd), flush=True)
        print(f"Running eval command (test):", flush=True)
