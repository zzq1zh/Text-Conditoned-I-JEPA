...
#   sbatch slurm_train.sh ./train_two_fusions_and_push.sh
...
case "${TARGET}" in
  two_fusions|two-stage|two_stage)
    ...
    ;;
  dinov3|dino|dino-v3)
    TRAIN_CMD=(uv run python train_csp_clipstyle.py --vision-backbone dino-v3)
    ;;
  ijepa|i-jepa)
    TRAIN_CMD=(uv run python train_csp_clipstyle.py --vision-backbone ijepa)
    ;;
  vjepa|v-jepa)
    TRAIN_CMD=(uv run python train_csp_clipstyle.py --vision-backbone v-jepa)
    ;;
  ./*.sh|*.sh)
